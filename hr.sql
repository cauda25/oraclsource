...
--			 해당 값이 0이면 0.0/1,0.09/2,0.20/3,03/4,0.40/5,0.42/6,0.44/그 외 0.45
SELECT
	E.LAST_NAME,
	E.SALARY,
...
SELECT MAX(SALARY),MIN(SALARY),MAX(SALARY)-MIN(SALARY) AS SAL_GAP
FROM EMPLOYEES e; 
...
-- EMPLOYEES,DEPARTMENT,LOCATINONS 조인
SELECT
	E.EMPLOYEE_ID ,
	E.LAST_NAME,
	D.DEPARTMENT_ID,
...
-- 각 부서별사원 수, 평균 연봉(소수점 2자리까지) 조회
-- 부서명,부서위치 아이디, 부서별 사원 수, 평균 연봉 출력
-- EMPLOYESS, DEPARTMENT 조인
SELECT 
	D.DEPARTMENT_NAME,
	D.LOCATION_ID,
	COUNT(E.EMPLOYEE_ID),
...
--Executive 부서에 근무하는 모든 사원들의 부서번호,이름(LAST_NAME),JOB_JD 조회
-- EMPLOYESS, DEPARTMENT 조인
SELECT
	D.DEPARTMENT_ID,
	E.LAST_NAME,
